...
HOW TO USE:
  - Metadata: python src/tools/domain_boundary_audit.py metadata
  - Run: python src/tools/domain_boundary_audit.py run --input-json '{"root": "src"}'
INPUT OBJECT:
...
FILE_METADATA = {
    ...
    "version": "1.0.0",
    "entrypoint": "src/tools/domain_boundary_audit.py",
    "category": "analysis",
...
class _DomainVisitor(ast.NodeVisitor):
    ...
    def __init__(self):
        self.import_domains: dict[str, str] = {}
        self.function_domains: dict[str, set[str]] = defaultdict(set)
        self.class_domains: dict[str, set[str]] = defaultdict(set)
        self.file_domains: set[str] = set()
        self.deep_accesses: list[dict] = []
        self.all_accesses: list[dict] = []
    ...
    def visit_Attribute(self, node: ast.Attribute):
        ...
        if len(chain) >= 2:
            ...
            for part in chain:
                if part in _ATTR_DOMAIN_MAP:
                    domains_in_chain.add(_ATTR_DOMAIN_MAP[part])
...
def _audit_file(path: Path, depth_warn: int = 3) -> dict | None:
    ...
    for name, domains in sorted(visitor.function_domains.items()):
        functions.append({"name": name, "domains": sorted(domains), "domain_count": len(domains)})

    classes = []
    for name, domains in sorted(visitor.class_domains.items()):
        classes.append({"name": name, "domains": sorted(domains), "domain_count": len(domains)})
...
def run(arguments: dict) -> dict:
    ...
    funcs_over = sum(1 for r in results for f in r["functions"] if f["domain_count"] > component_max)
    ...
    top_offenders_summary = [
        {"file": r["file"], "domain_count": r["file_domain_count"], "domains": r["file_domains"]}
