"""
FILE: mcp_server.py
ROLE: MCP stdio server for _constraint-registry.
WHAT IT DOES: Exposes constraint registry tools through MCP using the same
  underlying run(arguments) functions used by CLI execution.
HOW TO USE:
  - Start: python _constraint-registry/mcp_server.py
...
from tools.registry_build import FILE_METADATA as BUILD_METADATA, run as run_build
from tools.constraint_query import FILE_METADATA as QUERY_METADATA, run as run_query
...
TOOL_REGISTRY = {
    BUILD_METADATA["mcp_name"]: (BUILD_METADATA, run_build),
    QUERY_METADATA["mcp_name"]: (QUERY_METADATA, run_query),
}
